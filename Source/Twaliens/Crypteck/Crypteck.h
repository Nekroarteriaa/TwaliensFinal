...
protected:
	...
	/*                    */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crypteck")
	class APoolManager * StingPoolManager;
...
private:
	...
	//Timer For sting CD
	struct FTimerHandle StingCDTimer;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crypteck", meta = (AllowPrivateAccess = "true"))
	float DelayedCheckTime;
	
	bool DelayedToxicAction, DelayedFireAction;
	FVector SmashTarget, ToxicTarget, FireTarget;
	bool IsThereLocationConflict(const FVector & NewLocation, float Radius) const;
