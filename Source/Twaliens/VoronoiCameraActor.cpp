...
void AVoronoiCameraActor::BeginPlay()
{
	...
	SpringArmComponnt->TargetArmLength = TrgtArmLength;
	MaxCameraOffset = FVector(600.f, 600.f, 0.f);
...
void AVoronoiCameraActor::UpdatingFOV(float DefaultFOV, float PassedFOV, USceneCaptureComponent2D * SCC)
{
	if (PassedFOV != FOV)
	{
		PassedFOV = FOV;
		SCC->FOVAngle = PassedFOV;
	}
...
void AVoronoiCameraActor::ActivateDynamicSplitScreen()
{
	FVector _currentPlayer;
	FVector _playerLocation = _playerPawn->GetActorLocation();
	FVector _playerZeroNewRootPosition;
	FVector _playerOneNewRootPosition;
	UVoronoiCamMaths::GetVoronoiCameraLocation(MaxCameraOffset, _playerLocation, _playerPawn, _playerZeroNewRootPosition, _playerOneNewRootPosition);
	
	if (FollowedPlayerIndex == 0)
		_currentPlayer = _playerZeroNewRootPosition;

	else
		_currentPlayer = _playerOneNewRootPosition;


	StaticMeshComponnt->SetWorldLocation(_currentPlayer);
	/*if (!ActivateSplitScreen)
		IsWidgetReady = false;

	else if (ActivateSplitScreen)
	{
		UVoronoiCamMaths::GetVoronoiCameraLocation(MaxCameraOffset, _playerLocation, _playerPawn, _playerZeroNewRootPosition, _playerOneNewRootPosition);
		UpdateCameras(_playerZeroNewRootPosition, _playerOneNewRootPosition);
	}*/

}

void AVoronoiCameraActor::ActivateWidget(bool ActivateSplitScreen)
{
}
